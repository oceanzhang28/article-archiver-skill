...
⚠️ **Recompute the second-H1 offset AFTER building the result — a pre-edit offset goes stale (fired 2026-09-04 on 卡兹克入口, GPT-6 Astra piece)**: if you computed `second_h1 = content.find('# 入口', 1000)` on the ORIGINAL file, then inserted links into first_copy and concatenated `result = first_copy + content[second_h1:]`, the inserted characters shift the duplicate-structure boundary forward. Slicing `result[:second_h1]` with the stale offset then misreports a CORRECT first-copy-only insert as "1 in first copy / 1 in second copy" — the second insertion appears to have landed in the stale copy. Fix: re-run `actual_second = result.find('# 卡兹克知识入口', 1000)` on the final result before verifying (full-link total == 2, `result[:actual_second].count == 2`, `result[actual_second:].count == 0`). A misleading split verdict with a clean total of 2 is the signature of a stale boundary, not a misplaced insert.
...
### Product-promo / toolbook sales pages (198页可编辑PPT format, verified 2026-08-29)

高绩效HR also publishes pure product-sales pages for downloadable HR toolkits (e.g. 《HRBP人手一份的实操工具书（附下载）》 — 198-page editable PPT, 9大篇章/160+小节/34份工具表单, 限时首发68元). Body shape: header banner → product intro lines (`198页可编辑PPT` / `9大篇章…` / `标准价199元，限时首发68元` / `扫码立即购买` / `资料购买咨询：xxx`) → `— 资料预览 —` marker → N preview images (the ACTUAL content — toolbook pages) → tail repeat of the intro + `点击"阅读原文"立即购买实操手册`.

Handling:
- **Cut the header banner** at the first real product line (`body.find('198页可编辑PPT')`), same as the standard banner rule.
- **Cut the tail at the SECOND occurrence of the product marker**: `body[body.find('198页可编辑PPT'):body.find('198页可编辑PPT', body.find('198页可编辑PPT')+10)]`. The tail is a verbatim repeat of the intro + purchase CTA → drop entirely per the tail-repeat rule; do NOT compress-and-keep, it adds zero new info.
- **KEEP** the product intro lines (page count / chapters / price / purchase contact) and ALL preview images — they are the real content.
- **Add the slide-note** right after the `— 资料预览 —` marker: `> **注**：本文为高绩效HR公众号产品推广帖，正文为《XXX》预览图（198页可编辑PPT）。以下为从预览图中提取的内容框架，图片即工具书页面。`
- **OCR a sample of the preview images** to extract the toolbook's real structure (痛点-解决路径索引 / 场景化索引 / 自评维度 / 能力模型 / 实操模块如需求摸排、人才盘点、激励、冲突调解、人效指标) for 摘要/核心要点 — see ocr-fallback.md "Product-promo preview images". Do NOT fabricate content from the title alone; the OCR'd page framework is the only content source in a promo page.
- **Classification**: HRBP toolbook → HR知识/hrBP相关 (matches the existing HRBP作战手册 piece precedent; sales-page promos classify by the product's topic, not as course ads).
...
- ⚠️ **Data-heavy summaries (multi-benchmark / multi-stat articles) overshoot by 100+ chars — draft with ONE anchor, not full enumeration**. Real case (2026-08-17, 卡兹克 RealReplicaBench piece): first draft enumerated all three benchmarks with task counts + metrics (107 tasks / 23组42项 / 60.75% / 73.79% / 58.82% / 32.5%) → 358 chars, needed THREE trim rounds (358→297→275→246) and re-uploads. Same shape fired 2026-08-27 on the 卡兹克 GLM-5.3-Flash model-launch review (339→301→254→229): the first draft enumerated all five benchmarks (Terminal-Bench 84.3 / DeepSWE 63.4 / Toolathlon 78.4 / Agents' Last Exam 26.3 / Z.ai 29.0) plus pricing parentheticals; the winning draft kept ONE anchor (跑分逼近 Opus 4.8), the price-vs-DeepSeek story, and the author's own 68.1% eval result, and DROPPED the rest of the benchmark list entirely — for launch/price reviews the "wow" facts outrank the benchmark enumeration, and "drop entirely" is a valid third strategy alongside full-detail-anchor and one-line-compression. Draft strategy: fully detail ONE anchor example (the main benchmark the article is about), compress the others to one-line comparisons ("腾讯 E-Bench 最佳 avg@3 为73.79%，但3次全对稳定性仅58.82%"), and drop secondary metrics. Run `len()` on the 摘要 BEFORE the first upload, not after — a trim means a wasted PUT + GET verify cycle.
...
This skill's public home is `github.com/oceanzhang28/article-archiver-skill` (SSH push pre-configured via deploy key; remote `git@github.com:oceanzhang28/article-archiver-skill.git`). The installed local copy is the working superset — after improving it locally, push the changes back (`git add <changed files> && git commit && git push origin main`) so the public repo stays current. Keep `README.md` and `agents/openai.yaml` in the repo. The 商业知识 (business) category is part of the installed skill (see Classify Destination).

⚠️ **The local clone can DIVERGE from origin/main — push will be rejected as non-fast-forward** (fired 2026-08-26: origin/main had 2 extra commits — 穆胜咨询 classification + README localization — pushed from another machine, while the local clone sat behind). Resolution pattern:

1. **Inspect before touching anything**: `git log --oneline origin/main -5` and `git diff --stat <local-HEAD> origin/main` to see what origin added.
2. **Confirm the local installed files are SUPERSETS** of origin/main before overwriting: extract origin's version (`git archive origin/main | tar -x -C /tmp/gh_version`) and diff each file both ways (`diff local gh`). If the GitHub side only shows shorter/subset versions of local lines (no genuinely GitHub-only content), overwriting is safe. The installed copy has always been the strict superset in practice.
3. **Reset and re-apply**: `git reset --hard origin/main`, then copy ONLY the files that actually differ from the installed skill (SKILL.md, references/*) over the working tree.
4. **Stage selectively — never `git add -A`**: stage just the changed files (`git add SKILL.md references/...`) so repo-only files (README.md, agents/openai.yaml) never get clobbered by the copy step.
5. Commit + push; the reset makes it a clean fast-forward. After push, verify remote == installed: `git fetch origin && git show origin/main:SKILL.md | diff -q - <installed SKILL.md>`.
