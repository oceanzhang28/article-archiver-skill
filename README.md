...
| AI与HR应用 | `00知识库/AI与HR应用/` | `00知识库/AI与HR应用入口.md` |
...
Skill 优先判断文章的核心主题：当 AI 与 HR/组织/人才共同构成文章主旨时，归入 `AI与HR应用`；公众号账号映射用于主题不明确时的默认判断。仅顺带提及 AI 或 HR 的文章仍按主主题分类。
...
- `数字生命卡兹克` → `00知识库/卡兹克/`
- `HR实名俱乐部`、`高绩效HR` → 默认 `00知识库/HR知识/`；AI 与 HR 共同构成主旨时转入 `AI与HR应用`
- `润米商城` / 刘润商业分析类文章 → `00知识库/商业知识/`

新文章先按主题判定；账号映射只在主题不明确时提供默认目录。
